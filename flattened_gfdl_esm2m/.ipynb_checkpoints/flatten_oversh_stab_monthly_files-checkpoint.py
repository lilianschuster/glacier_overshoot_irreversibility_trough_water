...
path='/home/www/lschuster/provide/gfdl-esm2m_oversh_stab_uni_bern'
...
for var in ['t_ref','precip']:
    ...
    if scenario == 'oversh_T20OS15' and var =='t_ref':
        dsifs.to_netcdf(f'{path}/flat/gfdl-esm2m_{scenario}_glacier_invariant_flat.nc')
    ...
    for t in ds.time.values[:]:  # ds_merged_glaciers.time.values: .sel(time=slice('1901-04','2016-12'))
        ...
        sel.to_netcdf(f'{path}/flat/tmp/tmp_{scenario}_{var}_{t}.nc')
        #sel.to_netcdf(f'flat/tmp/tmp_{scenario}_{var}_{str(t[0])[:7]}.nc')

    
    for cent in [18,19,20,21,22,23,24]:
        with xr.open_mfdataset(f'flat/tmp/tmp_{scenario}_{var}_{cent}*.nc',
            ...
            dso_all2.to_netcdf(f'{path}/flat/tmp/gfdl-esm2m_{scenario}_{var_n}_global_monthly_flat_glaciers_{cent}.nc')
            
            
    dt = xr.open_mfdataset(path+f'/flat/tmp/gfdl-esm2m_{scenario}_{var_n}_global_monthly_flat_glaciers_*.nc')
    ...
    dt.to_netcdf(f'{path}/flat/2023.2/gfdl-esm2m_{scenario}_{var_n}_global_monthly_flat_glaciers.nc')
